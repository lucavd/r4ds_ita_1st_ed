...
```{r, risultati = "asis", echo = FALSE, messaggio = FALSE}
libreria(dplyr)
...
contributi <- contributi %>% 
  filter(!name %in% c("hadley", "Garrett", "Hadley Wickham",
                      "Garrett Grolemund")) %>% 
  arrange(nome) %>% 
  mutate(uname = ifelse(!grepl(" ", nome), paste0("@", nome), nome))

cat("I ringraziamenti vanno a tutti i collaboratori in ordine alfabetico: ")
