...
Esiste un altro modo per incorporare codice R in un documento R Markdown: direttamente nel testo, con:  `r inline()`. Questo può essere molto utile se si citano le proprietà dei dati nel testo. Per esempio, nel documento di esempio che ho usato all'inizio del capitolo, avevo:

> Abbiamo dati sui diamanti di `r inline('nrow(diamonds)')`. 
> Solo `r inline('nrow(diamonds) - nrow(smaller)')` sono più grandi di 
